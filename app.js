...
Return this exact JSON structure (all fields required):
{
  ...
  "narratives": {
    "market": <2-3 sentences on market size, segmentation, competitive positioning>,
    "defensibility": <2-3 sentences on moat strength and what is missing>,
    "traction": <2-3 sentences on evidence of demand and metric quality>,
    "businessModel": <2-3 sentences on revenue logic and path to durability>
  },
  "swot": [
    ...
    { "title": "Threats",       "body": <2-3 sentences on credible risks> }
  ],
  "recommendations": [
    <5 specific actionable strings>
  ]
}

Scoring: 80-100 fundable, 65-79 needs proof, 45-64 significant gaps, 0-44 too early.
Be specific. Name what is missing. Do not inflate scores.`;
...
function renderReport(report) {
  ...
  kpiGrid.innerHTML = Object.entries(report.categories)
    .map(([key, val]) => `<article class="kpi"><strong>${val}</strong><div>${labelFor(key)}</div></article>`)
    .join("");
